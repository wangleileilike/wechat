...
var config = {
    ...
    colors: ['#5B8FF9', '#5AD8A6', '#434348', '#90ed7d', '#f15c80', '#8085e9'],
...
function getDataRange(minData, maxData) {
    ...
    const min = findRange(minData, 'lower', limit) ;
    const max = findRange(maxData, 'upper', limit);
    return {
        minRange: min < 0 ? 0 : min,
        maxRange: max < 5 ? 5 : max,
    };
}
...
function findCurrentIndex(currentPoints, xAxisPoints, opts, config) {
    ...
    if (isInExactChartArea(currentPoints, opts, config)) {
        xAxisPoints.forEach(function (item, index) {
            if (currentPoints.x + offset > item + 25) {
                currentIndex = index;
            }
        });
    }
...
function getXAxisPoints(categories, opts, config, textSpace = 2) {
    var yAxisTotalWidth = config.yAxisWidth + config.yAxisTitleWidth;
    var spacingValid = opts.width - textSpace * config.padding - yAxisTotalWidth;
...
function drawToolTipSplitLine(offsetX, opts, config, context) {
    ...
    var endY = opts.height - config.padding - config.xAxisHeight - config.legendHeight - 35;
    context.beginPath();
...
function drawXAxis(categories, opts, config, context) {
    ...
    var _getTextXAxisPoints4 = getXAxisPoints(categories, opts, config, 6),
      xAxisTextPoints = _getTextXAxisPoints4.xAxisPoints
    ...
    if (config._xAxisTextAngle_ === 0) {
        ...
        categories.forEach(function (item, index) {
            ...
            context.fillText(item, xAxisTextPoints[index] + offset, startY + config.fontSize + 5);
        });
        context.closePath();
        context.stroke();
    } else {
        categories.forEach(function (item, index) {
            ...
            var _calRotateTranslate = calRotateTranslate(xAxisTextPoints[index] + eachSpacing / 2, startY + config.fontSize / 2 + 5, opts.height),
...
function drawCanvas(opts, context, bool = false) {
    context.draw(bool);
}
...
var Charts = function Charts(opts) {
    ...
    this.context1 = wx.createCanvasContext(opts.canvasTooltipId);
...
Charts.prototype.updateData = function () {
    ...
    this.context.clearRect(0, 0, 340, 200);
...
Charts.prototype.showToolTip = function (e) {
    ...
    if (this.opts.type === 'line' || this.opts.type === 'area') {
        ...
        this.context1.clearRect(0, 0, 340, 200);
        drawToolTipSplitLine(opts.tooltip.offset.x, opts, config, this.context1);
        drawCanvas(opts, this.context1, false);
    }
};
